...
var Match = reactExports.memo(function MatchImpl({ matchId }) {
  const router = useRouter();
  {
    const match2 = router.stores.activeMatchStoresById.get(matchId)?.get();
    if (!match2) {
      invariant();
    }
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-DWhFcLL-.js");
...
const manifest = {
  "536d5a6ebdda052d485a92852ccdbf3a8d0e2c6e508dc067c0e897a8a9e2bb66": {
    functionName: "submitContactForm_createServerFn_handler",
    importer: () => import("./contact.functions-DiYXB6-F.js")
  }
};
...
function getStartResponseHeaders(opts) {
  return mergeHeaders({ "Content-Type": "text/html; charset=utf-8" }, ...opts.router.stores.activeMatchesSnapshot.get().map((match) => {
...
var ROUTER_BASEPATH = "artisan-bakery-suite";
var SERVER_FN_BASE = "/artisan-bakery-suite/_serverFn/";
